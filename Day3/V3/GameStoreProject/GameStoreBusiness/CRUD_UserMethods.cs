...
namespace GameStoreBusiness
{
    public class CRUD_UserMethods
    {
        ...
        public void Create(string fullName, string username, string region, decimal wallet)
        {
            User newUser = new User() { FullName = fullName, Username = username, Region = region, Wallet = wallet };
        ...
        public void Update(string usernameOld, string usernameNew, string region, decimal wallet)
        {
            using (var db = new GameMarketContext())
            {
